...
1. 在**实例配置**下，对于**数据库实例类**，保留默认值，即 **db.t3.micro**。如果您的账户符合免费套餐的资格，则可以免费使用此数据库类。有关更多信息，请参阅 [AWS Free Tier](https://aws.amazon.com/free/)。
...
![在 RDS 控制台中创建 RDS 数据库。](https://docs.aws.amazon.com/zh_cn/AWSEC2/latest/UserGuide/images/tutorial-create-rds-database.gif)
...
![使用自动连接功能将 EC2 实例连接到 RDS 数据库。](https://docs.aws.amazon.com/zh_cn/AWSEC2/latest/UserGuide/images/tutorial-connect-new-ec2-rds.gif)
...
1. 在**连接和安全性**选项卡的**安全性**、**VPC 安全组**下，验证是否显示名为 **rds-ec2-{{x}}** 的安全组。
...
1. 在**安全详细信息**的**安全组**下，验证列表中是否存在名为 **ec2-rds-{{x}}** 的安全组。{{x}} 是一个数字。
